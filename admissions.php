<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Admissions - Pakistan Islamia Higher Secondary School</title>

    <link rel="stylesheet" href="./assets/css/global.css" />
    <link rel="stylesheet" href="./assets/css/admissions.css" />
    <link rel="shortcut icon" href="./assets/images/global/logo_small.png" type="image/x-icon" />
  </head>
  <body>
    <?php include('./assets/php/header.php')?>
    <main>
      <!-- Banner -->
      <section id="admissions-banner" class="subpage-banner">
        <div class="banner-container">
					<h1>Admissions</h1>
				</div>
      </section>
      <div id="nav-breadcrumbs" class="nav-breadcrumbs">
        <ul>
          <li><a href="./index.php">HOME</a></li>
          <li><a href="javascript:window.location.reload(true)">About Us</a></li>
        </ul>
      </div>

      <!-- Admissions -->
      <section id="admissions" class="admissions">
        <div class="admissions-container">
          <div class="admissions-overview">
            <div class="content">
              <div class="h1-border">
                <span></span>
                <h1>Admissions Overview</h1>
              </div>
              <p>
                We understand that choosing the right school for your child is a
                huge decision, so our admissions process is simple,
                straightforward and designed to give assurance to you and your
                child that PIHSS is the right choice for your family.
              </p>

              <p>
                <strong>NOTE:</strong>
              </p>

              <ul class="note">
                <li>
                  Parents seeking admission for their children should contact
                  the school during Jan-Feb.
                </li>
                <li>
                  Admission depends on the availability of seats. Applicants are
                  contacted only if seats are available.
                </li>
                <li>
                  Parents should bring all the required documents on the day of
                  interview/assessment test.
                </li>
                <li>
                  Parents have to submit the due fees along with any pending
                  documents within 3 days.
                </li>
              </ul>
            </div>
            <div class="internal-nav">
              <div>
                <h2>Admissions</h2>
                <ul>
                  <li><a href="#admissions">Admissions</a></li>
                  <li><a href="#fee-structure">Fee Structure</a></li>
                  <li><a href="#registration">Registration</a></li>
                </ul>
              </div>
            </div>
          </div>

          <!-- Admissions Process -->
          <div class="admissions-process">
            <h2>Admissions Process</h2>
            <ul>
              <li>
                <span class="card-number">1</span>
                <div class="card-content">
                  <p>KG1 admission: Assessment Interviews</p>
                  <p>
                    KG2 - Grade 10 admission: Entrance tests in English,
                    Mathematics & Science
                  </p>
                </div>
              </li>
              <li>
                <span class="card-number">2</span>
                <div class="card-content">
                  <p>Answer papers will NOT be shown to parents</p>
                </div>
              </li>
              <li>
                <span class="card-number">3</span>
                <div class="card-content">
                  <p>
                    Parents will be notified by admin office via SMS/phone
                    call/website for the test date
                  </p>
                </div>
              </li>
              <li>
                <span class="card-number">4</span>
                <div class="card-content">
                  <p>
                    Successful applicants will be notified by the Admin office
                    via SMS/phone call/website and pasted on the school’s notice
                    board
                  </p>
                </div>
              </li>
            </ul>
          </div>

          <!-- Admissions Requirements -->
          <div class="admissions-requirements">
            <h1>Requirements</h1>
            <h2>Documents Required</h2>
            <div class="required-documents-table table-wrapper">
              <table>
                <thead>
                  <tr>
                    <th scope="col">S. No.</th>
                    <th scope="col">Document</th>
                    <th scope="col">Original</th>
                    <th scope="col">Copy</th>
                    <th scope="col">Class</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td data-label="S. No.">1.</td>
                    <td data-label="Document">Passport</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">KG - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">2.</td>
                    <td data-label="Document">Emirates ID</td>
                    <td data-label="Original">Yes</td>
                    <td data-label="Copy">Yes (2 Copies)</td>
                    <td data-label="Class">KG - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">3.</td>
                    <td data-label="Document">Birth Cerificate</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">KG 1 - Class 3</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">4.</td>
                    <td data-label="Document">Photograph (White Background)</td>
                    <td data-label="Original">4</td>
                    <td data-label="Copy">-</td>
                    <td data-label="Class">KG 1 - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">5.</td>
                    <td data-label="Document">Vaccination Card</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">KG 1 - Class 10</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">6.</td>
                    <td data-label="Document">
                      Medical File (from previous school)
                    </td>
                    <td data-label="Original">Yes</td>
                    <td data-label="Copy">No</td>
                    <td data-label="Class">KG 2 - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">7.</td>
                    <td data-label="Document">Health Card/Insurance Card</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">Class 11 - 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">8.</td>
                    <td data-label="Document">Father's/Guardian's Passport</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">KG 1 - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">9.</td>
                    <td data-label="Document">
                      Father's/Guardian's Emirates ID
                    </td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">KG 1 - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">10.</td>
                    <td data-label="Document">Class 9 Marks Sheet</td>
                    <td data-label="Original">No</td>
                    <td data-label="Copy">Yes</td>
                    <td data-label="Class">Class 11</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">11.</td>
                    <td data-label="Document">School Leaving Certificate</td>
                    <td data-label="Original">Yes</td>
                    <td data-label="Copy">No</td>
                    <td data-label="Class">KG 2 - Class 12</td>
                  </tr>
                  <tr>
                    <td data-label="S. No.">12.</td>
                    <td data-label="Document">
                      Report Card from Previous School
                    </td>
                    <td data-label="Original">Yes</td>
                    <td data-label="Copy">No</td>
                    <td data-label="Class">KG 2 - Class 12</td>
                  </tr>
                </tbody>
              </table>
            </div>

            <div class="note">
              <p>
                * Please note that for students coming from Pakistan, School
                Leaving Certificate & Report Cards must be attested from:
              </p>
              <ol>
                <li>District Education Officer</li>
                <li>Ministry of Foreign Affairs in Pakistan</li>
                <li>U.A.E. Embassy in Pakistan</li>
              </ol>
            </div>

            <!-- Seems to be the expected ages of students of a particular grade -->
            <h2>Age Requirement</h2>
            <div class="table-wrapper">
              <div class="age-requirement">
                <table>
                  <tr>
                    <th scope="col">Class</th>
                    <th scope="col">Age</th>
                  </tr>
                  <tr>
                    <td>KG 1</td>
                    <td>4 Years 11 Montds</td>
                  </tr>
                  <tr>
                    <td>KG 2</td>
                    <td>5 Years 11 Montds</td>
                  </tr>
                  <tr>
                    <td>1</td>
                    <td>6-8 Years</td>
                  </tr>
                  <tr>
                    <td>2</td>
                    <td>7-9 Years</td>
                  </tr>
                  <tr>
                    <td>3</td>
                    <td>8-10 Years</td>
                  </tr>
                  <tr>
                    <td>4</td>
                    <td>9-11 Years</td>
                  </tr>
                  <tr>
                    <td>5</td>
                    <td>10-12 Years</td>
                  </tr>
                  <tr>
                    <td>6</td>
                    <td>11-13 Years</td>
                  </tr>
                  <tr>
                    <td>7</td>
                    <td>12-14 Years</td>
                  </tr>
                  <tr>
                    <td>8</td>
                    <td>13-15 Years</td>
                  </tr>
                  <tr>
                    <td>9</td>
                    <td>14-16 Years</td>
                  </tr>
                  <tr>
                    <td>10</td>
                    <td>15-17 Years</td>
                  </tr>
                  <tr>
                    <td>11</td>
                    <td>16-18 Years</td>
                  </tr>
                  <tr>
                    <td>12</td>
                    <td>17-19 Years</td>
                  </tr>
                </table>
              </div>
            </div>
          </div>
        </div>
      </section>

      <!-- Fee Structure -->
      <section id="fee-structure" class="fee-structure">
        <h1>Fee Structure</h1>
        <p>
          Fee is applicable for 10 months i.e. from April to March (Excluding
          July & August). In case if a student joins during the session, fee
          will be applicable from start of the academic year i.e. from April.
        </p>

        <!-- Table for Fees -->
        <div class="table-wrapper">
          <table>
            <tr>
              <th scope="col">Class</th>
              <th scope="col">Fee</th>
            </tr>
            <tr>
              <td>KG 1 - Grade 3</td>
              <td>AED 4,500 / Annum</td>
            </tr>
            <tr>
              <td>Grade 4 - Grade 5</td>
              <td>AED 4,700 / Annum</td>
            </tr>
            <tr>
              <td>Grade 6 - Grade 8</td>
              <td>AED 5,200 / Annum</td>
            </tr>
            <tr>
              <td>Grade 9</td>
              <td>AED 5,800 / Annum</td>
            </tr>
            <tr>
              <td>Grade 10</td>
              <td>AED 6,000 / Annum</td>
            </tr>
            <tr>
              <td>Grade 11 - Grade 12</td>
              <td>AED 8,000 / Annum</td>
            </tr>
          </table>
        </div>

        <div>
          <h3>Note:</h3>
          <ul>
            <li>
              Federal Board Registration Fee will be charged for students of
              grade 9 & 11 students.
            </li>
            <li>
              Federal Board Examination Fee will be charged for students of
              Grade 9 - 12.
            </li>
            <li>
              The fee rate is subject to enhancement anytime after it is duly
              approved by the Ministry of Education.
            </li>
            <li>Fee should be paid in advance before 10th of every month.</li>
            <li>
              Non-payment of fee will result in the name to be forwarded to the
              Ministry of Education for cancellation of admission as per rules.
            </li>
          </ul>
        </div>

        <div class="discounts-refunds">
          <div>
            <h2>Discount</h2>
            <p>Following discounts are available in tuition fee:</p>
            <ul>
              <li>
                50% discount on tuition fee for orphans subject to provision of
                required documents.
              </li>
              <li>
                (KG 1 - Grade 5) AED20 discount on tuition fee for 3rd child
                onwards (applicable to the student studying in lowest class)
              </li>
              <li>
                (Grade 6 - Grade 10) AED30 discount on tuition fee for 3rd child
                onwards
              </li>
              <li>
                (Grade 11 - Grade 12) AED60 discount on tuition fee if 2 or more
                children are in grade 11 or 12.
              </li>
            </ul>
          </div>

          <div>
            <h2>Refunds</h2>
            <p>Refunds will be done as follows:</p>
            <ul class="note">
              <li>Full refund if the student does not attend the class.</li>
              <li>
                1 month fee will be deducted if the student attends the school
                for 2 weeks or less.
              </li>
              <li>
                2 months fee will be deducted if the student attends the school
                for more than 2 weeks and less than 1 month.
              </li>
              <li>
                3 months fee will be deducted if the student attends the school
                for more than 1 month.
              </li>
            </ul>
          </div>
        </div>
      </section>

      <section id="registration" class="registration">
        <h1>Registration</h1>
        <h2>Student Registration Form</h2>
        <form class="registration-form" id="registration-form" action="./assets/php/registration-submit.php" method="POST">
          <div class="form-item">
            <label for="student-name">Student Name</label>
            <input type="text" id="student-name" name="student-name" spellcheck="false" autocomplete="off" required>
          </div>
          <div class="form-item">
            <label for="gender">Gender</label>
            <div class="radio-group">  
              <div class="input-group">
                <!-- <label for="gender-female"></label> -->
                <input id="gender-female" type="radio" name="gender" value="f" required>
                Female
              </div>
              
              <div class="input-group">  
                <!-- <label for="gender-female"></label> -->
                <input id="gender-male" type="radio" name="gender" value="m" required>
                Male
              </div>
            </div>
          </div>
          <div class="form-item">
            <label for="date-of-birth">Date of Birth</label>
            <input type="date" name="date-of-birth" required>
          </div>
          <div class="form-item">
            <label for="eid-number">Emirates ID Number</label>
            <input type="text" name="eid-number" required autocomplete="off" minlength="15" maxlength="15">
          </div>
          <div class="form-item">
            <label for="eid-issue">EID Issue Date</label>
            <input type="date" name="eid-issue" required>
          </div>
          <div class="form-item">
            <label for="eid-expiry">EID Expiry Date</label>
            <input type="date" name="eid-expiry" required>
          </div>
          <div class="form-item">
            <label for="eid-copy-front">Upload EID - Frontview</label>
            <label class="file-upload">
              <div class="upload-button">Add Files</div>
              <div class="image-drop">Drop files here to add</div>
              <input type="file" name="eid-copy-front" class="image-dropzone" required>
            </label>
          </div>
          <div class="form-item">
            <label for="eid-copy-back">Upload EID - Backview</label>
            <label class="file-upload">
              <div class="upload-button">Add Files</div>
              <div class="image-drop">Drop files here to add</div>
              <input type="file" name="eid-copy-back" id="" class="image-dropzone" required>
            </label>
          </div>
          <div class="form-item form-item-file">
            <label for="passport-copy">Passport Copy</label>
            <label class="file-upload" for="">
              <div class="upload-button">Add Files</div>
              <div class="image-drop">Drop files here to add</div>
              <input type="file" name="passport-copy" id="" class="image-dropzone" required>
            </label>
          </div>
          <div class="form-item">
            <label for="student-class">Student Class</label>
            <input type="text" name="student-class" id="" required>
          </div>
          <div class="form-item form-item-empty">
            <label for="form-submit">Form Buttons</label>
            <input class="form-button form-submit" type="submit" value="Submit">
            <input class="form-button form-reset" type="reset" value="Clear All" required>
          </div>
        </form>
      </section>
    </main>

    <!-- Footer -->
    <?php include('./assets/php/footer.php')?>

    <script src="assets/js/global-scripts.js"></script>
  </body>
</html>